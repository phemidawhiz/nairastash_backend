...
if(!is_array($decoded)) {
  ...
  if (isset($_SESSION['user_id'], $_SESSION['email'], $_SESSION['urole'], $_SESSION['login_string'], $_SESSION['regcode'])) {
    ...
    $transcode = rand(10,2999) . strtolower(substr($reqmememail,3,1)) . rand(3000,6999) . strtolower(substr($reqmememail,-1,1)) . rand(7000,9999) ;
      ...
      $sql1 = "SELECT pendingpayment, walletbalance FROM members WHERE regcode = '$reqmemcode' ";
      $result1 = mysqli_query($mysqli,$sql1) or die(mysqli_error($mysqli));
      $data = mysqli_fetch_assoc($result1);

      if($result1) {
        if($data['pendingpayment'] == 0 ) {
          if($data['walletbalance'] >= $reqamount && $reqamount >= 1000 ) {
            ...
            if($insert_stmt->execute()) {
              ...
              $sql = "UPDATE members SET pendingpayment = '$reqamount', walletbalance = walletbalance - $reqamount WHERE regcode='$reqmemcode'";
              $result = mysqli_query($mysqli, $sql) or die("Unable to update member table: " . mysqli_error($mysqli));

              if($result) {
                $message1 = "You have successfully request network bonus payment. Your request will be processed within 12 hours and payment will be made within 24 hours.";
                $notiftype1 = "request";
            ...
            if($data['walletbalance'] < $reqamount) {
              $jsonArray = array("message" => "Insufficient balance!",  "status" => false);
              $jsonData = json_encode($jsonArray);
              echo $jsonData;
          ...
          $jsonArray = array("message" => "You have a pending payment request. Only one request can be made at a time. Please try again later", "status" => false);
          $jsonData = json_encode( $jsonArray);
          echo $jsonData;
          die();
        }
