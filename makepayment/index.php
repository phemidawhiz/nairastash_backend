    ...
    if($_SESSION['urole'] >= 4) {
      ...
      if(isset($_GET["memcode"])) {
        ...
        $transcode = rand(10,2999) . strtolower(substr($adminEmail,3,1)) . rand(3000,6999) . strtolower(substr($adminEmail,-1,1)) . rand(7000,9999) ;
          ...
          $sql1 = "SELECT pendingpayment, walletbalance FROM members WHERE regcode = '$paymentMemberCode' ";
          ...
          if($result1) {
            if($data['pendingpayment'] != 0 ) {
              ...
              if($pendingpayment >= 1000 ) {
                ...
                $transtype = 1;
                $insert_stmt->bind_param('ssisi', $paymentMemberCode, $adminEmail, $pendingpayment, $transcode, $transtype);

                // Execute prepared query
                if($insert_stmt->execute()) {
                  //Update pendingpayment field of member table
                  $sql = "UPDATE members SET pendingpayment = pendingpayment - $pendingpayment, received = received + $pendingpayment WHERE regcode='$paymentMemberCode'";
                  ...
                  if($result) {
                    ...
                    $notiftype1 = "payment";
                    $message2 = "Your payment request has been authorised. Check your bank account from time to time. If you're not credited after 24 hours from now, please contact us ";
                    $notiftype2 = "payment";
